...
void BUS_I2C_ALIFG(void) __ctl_interrupt{
...
void BUS_I2C_NACKIFG(void) __ctl_interrupt{
...
void BUS_I2C_STTIFG(void) __ctl_interrupt{    //start condition received
...
void BUS_I2C_STPIFG(void) __ctl_interrupt{    //Stop condition received
  //check if we are master
  if(UCB0CTLW0&UCMST){
...
void BUS_I2C_RXIFG3(void) __ctl_interrupt{    //Slave 3 RXIFG
  //check buffer size
...
void BUS_I2C_TXIFG3(void) __ctl_interrupt{    //Slave 3 TXIFG
  //no data to send so send dummy data
  UCB0TXBUF=BUS_I2C_DUMMY_DATA;
}

void BUS_I2C_RXIFG2(void) __ctl_interrupt{    //Slave 2 RXIFG
  //check buffer size
  if(arcBus_stat.i2c_stat.rx.idx>=sizeof(I2C_rx_buf[0].dat)){
    //receive buffer is full, send NACK
    UCB0CTL1|=UCTXNACK;
...
void BUS_I2C_TXIFG2(void) __ctl_interrupt{    //Slave 2 TXIFG
}

void BUS_I2C_RXIFG1(void) __ctl_interrupt{    //Slave 1 RXIFG
}

void BUS_I2C_TXIFG1(void) __ctl_interrupt{    //Slave 1 TXIFG
}

void BUS_I2C_RXIFG0(void) __ctl_interrupt{    //Data receive in master mode and Slave 0 RXIFG
...
void BUS_I2C_TXIFG0(void) __ctl_interrupt{    //Data transmit in master mode and Slave 0 TXIFG
      //check master status 
      if(arcBus_stat.i2c_stat.tx.stat==BUS_I2C_MASTER_PENDING){
...
void BUS_I2C_BCNTIFG(void) __ctl_interrupt{    //Byte Counter Zero
}

void BUS_I2C_CLTOIFG(void) __ctl_interrupt{    //Cock low timeout
...
void BUS_I2C_BIT9IFG(void) __ctl_interrupt{    //9th bit interrupt
}
